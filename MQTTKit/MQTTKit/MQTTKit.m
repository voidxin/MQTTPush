...
- (BOOL)enableBackgrounding {
    ...
    BOOL res;
    if (readStream) {
        res = CFReadStreamSetProperty(readStream, kCFStreamPropertyShouldCloseNativeSocket, kCFBooleanFalse);
        if (!res) {
            goto __EnableBackgrounding_Failed;
        }
        res = CFReadStreamSetProperty(readStream, kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeVoIP);
        if (!res) {
            goto __EnableBackgrounding_Failed;
        }
    }
    ...
    res = CFReadStreamOpen(readStream);
    if (!res) {
        goto __EnableBackgrounding_Failed;
    }
